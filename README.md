...
## Installation
Obtain the latest version of Python 3 (version 3.8.5 at the time of this writing) from official [website](https://www.python.org). Once downloaded install normally on your machine.
...
After verifying the Python 3 installation, **_upgrade pip_** and **_install Pillow_** using following commands.
```bash
...
Once the environment variable are updated, check Python version using **Command Prompt** or **PowerShell**, by executing 
```
python --version
```
...
## Usage
Download the [imgconvert.py](https://github.com/randomsdeveloper/imgconvert/blob/master/imgconvert.py) file and place in the directory of choice, where it is easy to access the contents of directory in which image files are to stored for conversion.
...
_**Note:** If single file is choosen to convert, the prompt only asks about output file type as it knows input file type already. However, in case of directory, it needs to know the output type as well input file type to select multiple files for conversion_
